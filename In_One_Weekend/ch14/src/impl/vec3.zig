...
pub const vec3 = struct {
    ...
    pub fn refract(uv: vec3, n: vec3, etai_over_etat: f64) vec3 {
        const cos_theta = @min(vec3.dot(vec3.neg(uv), n), 1.0);

        const r_out_perp = n.mul(cos_theta).add(uv).mul(etai_over_etat);
        const r_out_parallel = vec3.mul(n, -@sqrt(@abs(1.0 - r_out_perp.length_squared())));
    ...
    pub fn random_in_unit_disk() vec3 {
        while (true) {
            const p = vec3.init(Rtweekend.random_double_minmax(-1, 1), Rtweekend.random_double_minmax(-1, 1), 0);
